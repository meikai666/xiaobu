...
- **2026-03-06**：知乎专栏文章格式问题——通过 browser type 直接输入 Markdown 符号（##、**、---）会显示成纯文本，需要用工具栏按钮或导入功能。
- **2026-03-06**：创建了完整的闭环工作流程（WORKFLOW.md），包含 8 个阶段：接收任务→查 skills→制定计划→执行→检查结果→记录教训→Git 提交→定期复盘。
- **2026-03-06**：学会了制作工作流的方法（WORKFLOW-CREATOR.md），并创建了通用工作流 WORKFLOW-RDSR.md（需求→拆解→输出→复盘）。以后任何任务都按这个流程执行。
- **2026-03-06**：小凯确认工作流程——以后每次接受新任务都按这个流程处理，中间有需要沉淀 skill 的就顺手沉淀。
